...
public class BaosService {
	...
	public static class Timer {
		...
		// ??? use zoned, or Instant, or local date/time with default time zone
...
//	private static final int ProtocolV2 = 2;
	...
	private static boolean isSupportedService(final int service) {
		if (service == DatapointValueIndication || service == ServerItemIndication)
			return true;
		final int svc = service & ~ResponseFlag;
	...
	public static BaosService getServerItem(final int startItemId, final int items) {
		return new BaosService(GetServerItem, startItemId, items);
	}

	static BaosService setServerItem(final Map<Integer, byte[]> items) {
		return new BaosService(SetServerItem, items);
	}

	static BaosService getDatapointDescription(final int startDatapointId, final int datapoints) {
		return new BaosService(GetDatapointDescription, startDatapointId, datapoints);
	}

	static BaosService getDatapointDescriptionString(final int startDatapointId, final int datapoints) {
	...
	static BaosService setDatapointHistoryCommand(final int startDatapoint, final int datapoints, final HistoryCommand command) {
		return new BaosService(SetDatapointHistoryCommand, startDatapoint, datapoints, (byte) command.ordinal());
	}

	static BaosService getDatapointHistoryState(final int startDatapoint, final int datapoints) {
		return new BaosService(GetDatapointHistoryState, startDatapoint, datapoints);
	}

	static BaosService getDatapointHistory(final int startDatapoint, final int datapoints, final Instant start, final Instant end) {
	...
	public static BaosService setTimer(final List<Timer> timers) {
		return new BaosService(SetTimer, timers.get(0).id(), timers.size(), timerByteArray(timers));
	}
	...
	@Override
	public String toString() {
		final String response = (subService & ResponseFlag) != 0 ? " response" : "";
		if (items == 0)
			return Integer.toHexString(subService) + response + " item " + start + " error: "
					+ ErrorCode.of(data[0] & 0xff);

		final var s = Integer.toHexString(subService) + response + " start " + start + " items " + items;
	...
	private static byte[] timerByteArray(final List<Timer> timers) {
		final var os = new ByteArrayOutputStream();
		timers.forEach(timer -> os.writeBytes(timer.toByteArray()));
